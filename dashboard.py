...
class Dashboard:
    def __init__(self):
        self.snow_connection = Snowflake().connect()

    def load_data(self, ticker):
        cs = self.snow_connection.cursor()
    ...
    def plot(self):
        ...
        try:
            with st.spinner(f"Loading data for {ticker_symbol}..."):
                df = self.load_data(ticker_symbol)
            ...
            fig = self.plot_stock(df, ticker_symbol)
    ...
    def plot_stock(self, df, ticker):
        ...
        signal_colors = df['Signal_Strength'].apply(self.get_color)
